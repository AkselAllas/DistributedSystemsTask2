...
export const createDockerContainer = async (node: ProcessNode) => {
  try {
    await asyncExec(
      `export JSON={\\"id\\":${node.id},\\"name\\":\\"${node.name}\\",\\"electionCount\\":${node.electionCount},\\"time\\":\\"${node.time}\\",\\"allNodeIds\\":[${node.allNodeIds}],\\"isCoordinator\\":${node.isCoordinator},\\"isElecting\\":${node.isElecting},\\"originalTime\\":\\"${node.time}\\",\\"electionStartedBy\\":${node.electionStartedBy},\\"mainProcessIpAddress\\":\\"${node.mainProcessIpAddress}\\",\\"isFrozen\\":${node.isFrozen}} ; docker run --network dst2 --ip 172.13.42.${node.id} --name dst2-${node.id} -dit dst2 /usr/local/bin/node /app/src/node.js $JSON`,
    );
